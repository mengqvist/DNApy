...
class CodonView(DNApyBaseDrawingClass):
	...
	The information generated by user actions is used to compute the ambigous codon.
	'''
...
############### Done setting required methods #######################
	...
	def Draw(self, dc):
		...
		This method is responsible for drawing the entire user interface, with the exception of buttons that I add later.
		'''
		...
		coding_nucleotide_color = '#4B4424' #for coloring the nucleotides encoded by the ambiguous codon
		...
		#now draw the highlighted amino acid (the one that the mouse hovers above).
...
class CodonButtonWrapper(DNApyBaseClass):
	...
	def InputCodonOnText(self, evt):
		'''
		When text is entered into the text field, check if it is a valid ambiguous codon.
